...
class Side2BaseHead(AnchorHead):
    ...
    def __init__(self,
        ...
        super(Side2BaseHead, self).__init__(
            num_classes,
            in_channels,
    ...
    def forward(self, feats):
        ...
        for i, cls_conv in enumerate(self.cls_convs):
            ...
            if i < 2:
                nowPrevClsFeat = self.uppers[i](cls_feat.detach().clone())
                nowPrevClsFeat = F.dropout2d(nowPrevClsFeat, 0.5, False)
                # nowPrevClsFeat = F.adaptive_max_pool2d(output_size=(cls_feat.shape[2:]), input=nowPrevClsFeat)
                prev_cls_feats.append(nowPrevClsFeat)
        scale_feat.append(cls_feat)
        for i, reg_conv in enumerate(self.reg_convs):
            reg_feat = reg_conv(reg_feat)
            if i < 2: 
                nowPrevRegFeat = self.uppers[i](reg_feat.detach().clone())
                nowPrevRegFeat = F.dropout2d(nowPrevRegFeat, 0.5, False)
                # nowPrevRegFeat = F.adaptive_max_pool2d(output_size=(reg_feat.shape[2:]), input=nowPrevRegFeat)
        ...
        for feat in feats[1:]:
            ...
            for i, cls_conv in enumerate(self.cls_convs):
                if i < 2:
                    ...
                    cls_feat = self.relus[i](cls_feat)
                cls_feat = cls_conv(cls_feat)
                if i < 2:
                    nowPrevClsFeat = self.uppers[i](cls_feat.detach().clone())
                    nowPrevClsFeat = F.dropout2d(nowPrevClsFeat, 0.5, False)
            ...
            for i, reg_conv in enumerate(self.reg_convs):
                if i < 2:
                    ...
                    reg_feat = self.relus[i](reg_feat)
                reg_feat = reg_conv(reg_feat)
                if i < 2:
                    nowPrevRegFeat = self.uppers[i](reg_feat.detach().clone())
                    nowPrevRegFeat = F.dropout2d(nowPrevRegFeat, 0.5, False)
                    prev_reg_feats.append(nowPrevRegFeat)
                if i == self.stacked_convs-1:
                    scale_feat.append(reg_feat)
            scale_feats.append(scale_feat)
            # feats
